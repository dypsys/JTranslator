...
use Translator\Debug\Handler\ProductionHandler;
use Translator\Debug\Database\DatabaseDebugger;
use Translator\Debug\Format\Html\SqlFormat;
use Translator\Debug\Format\Html\TableFormat;
...
/**
 * Class TranslatorDebugger.
 *
 * @since  1.0
 */
class TranslatorDebugger implements LoggerAwareInterface
{
    /**
     * Application object.
     *
    ...
    public function __construct(Container $container)
    {
        ...
        $this->profiler = new Profiler('Translator');
            // $container->get('app')->get('debug.system') ? new Profiler('Translator') : null;
    ...
    public function getOutput()
    {
        ...
        if ($this->application->get('debug.system'))
        {
            ...
            $debug[] = '<h3>' . Text::_('Profile') . '</h3>';
            $debug[] = $this->renderProfile();
            $debug[] = '</div>';

            $debug[] = '<div id="dbgUser">';
            $debug[] = '<h3>' . Text::_('User') . '</h3>';
        ...
        if ($this->application->get('debug.language'))
        {
            ...
            $debug[] = '<h3>' . Text::_('Language Strings') . '</h3>';
            $debug[] = $this->renderLanguageStrings();
            $debug[] = '</div>';

            $debug[] = '<div id="dbgLanguageFiles">';
            $debug[] = '<h3>' . Text::_('Language Files') . '</h3>';
    ...
    private function getNavigation()
    {
        ...
        $navigation[] = '<a class="brand" href="#top" class="hasTooltip" title="' . Text::_('Go up') . '">'
...
//                . '</a></li>';
            ...
            $navigation[] = '<li class="hasTooltip"'
                . ' title="' . Text::plural('One database query %d database queries', $count). '">'
                . '<a href="#dbgDatabase"><i class="icon icon-database"></i> '
                . $this->getBadge($count)
                . '</a></li>';
        ...
        if ($this->application->get('debug.system'))
        {
            ...
            // $title = $project ? $project->title : Text::_('No Project');
            $title = Text::_('No Project');
            ...
            $navigation[] = '<li class="hasTooltip"'
                ...
                . ($user && $user->username ? $user->username : Text::_('Guest'))
                . '</span></a></li>';

            // $navigation[] = '<li class="hasTooltip"'
            //    . ' title="' . Text::_('Project') . '">'
    ...
    public function renderException(\Exception $exception, array $context = array())
    {
        ...
        $view = new \Translator\View\TranslatorDefaultView;
    ...
    protected function renderDatabase()
    {
        ...
        $debug[] = Text::plural('One database query %d database queries', count($dbLog));
    ...
    protected function renderLanguageStrings()
    {
        ...
        $items = Text::_('processedItems');

        $html[] = '<table class="table table-hover table-condensed">';
        $html[] = '<tr>';
        $html[] = '<th>' . Text::_('String') . '</th><th>' . Text::_('File (line)') . '</th><th></th>';
        $html[] = '</tr>';
